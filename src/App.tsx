...
export default function App (): JSX.Element {
  ...
  return (
    <div className="container mx-auto mt-5 p-4 bg-white">
       ...
       { items.map((item) => {
         return (
          <div className="flex pt-5" key={item.link}>
          <div className="flex-none w-48 mr-4">
            <img src={(item.thumbnail != null) ? item.thumbnail : ''} alt={(item.title != null) ? item.title : ''} />
          </div>
          <div className="flex-auto">
            <h2 className="text-lg font-bold">{item.title}</h2>
            <p className="text-base mt-3 mb-1">{item.description}</p>
            <a target="_blank" rel="noreferrer" href={(item.link != null) ? item.link : ''}>
